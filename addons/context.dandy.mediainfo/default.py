...
def check_is_edit(title):
    ...
    if ADDON.getSetting('is_edit') == "true":
        ...
        for pattern in patterns:
            match = re.compile(pattern.decode('utf-8')).match(title.decode('utf-8'))
            if match:
               isedit = True
               break
    return isedit
...
def get_media_title():
    ...
    for pattern in patterns:
        title = re.compile(pattern.decode('utf-8')).sub("", title.decode('utf-8')).strip()
...
def get_media_meta_year():
    title = get_title()
    pattern = r"[([]([12][90]\d\d)[]), ]"
    match = re.compile(pattern.decode('utf-8')).search(title.decode('utf-8'))
...
def select_media(data):
    ...
    media = []
    for item in data:
        media.append("%s/%s [%s]" % (item["title"], item["original_title"], get_date_by_tag(item)))
...
def get_media_meta_movie_id():
    ...
    if response and (not (response == "Empty")):
        clear_response(response['results'])
